...
#include "indicator_logic.h"           /* Model header file */
...
void rt_OneStep(void)
{
  ...
  /* Check for overrun */
  if (OverrunFlag) {
    rtmSetErrorStatus(indicator_logic_M, "Overrun");
    return;
  }
...
int_T main(int_T argc, const char *argv[])
{
  ...
  fflush((NULL));
  while (rtmGetErrorStatus(indicator_logic_M) == (NULL)) {
    /*  Perform application tasks here */
  }

  /* Terminate model */
  indicator_logic_terminate();
  return 0;
}
